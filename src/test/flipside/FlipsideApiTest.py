...
class FlipsideApiTest(unittest.TestCase):
    ...
    def test_execute_get_tx_eth(self):
        sql = self.flipside_api.get_eth_transactions_sql_query(self.list_unique_address, limit=10)
        df = self.flipside_api.execute_query(sql)
        self.assertEqual(10, df.shape[0])
    def test_execute_get_tx_polygon(self):
        sql = self.flipside_api.get_polygon_transactions_sql_query(self.list_unique_address, limit=10)
        df = self.flipside_api.execute_query(sql)
        self.assertEqual(10, df.shape[0])
    def test_execute_get_tx_optimism(self):
        sql = self.flipside_api.get_optimism_transactions_sql_query(self.list_unique_address, limit=10)
        df = self.flipside_api.execute_query(sql)
        self.assertEqual(10, df.shape[0])
    def test_execute_get_tx_arbitrum(self):
        sql = self.flipside_api.get_arbitrum_transactions_sql_query(self.list_unique_address, limit=10)
        df = self.flipside_api.execute_query(sql)
        self.assertEqual(10, df.shape[0])
    def test_execute_get_tx_gnosis(self):
        sql = self.flipside_api.get_gnosis_transactions_sql_query(self.list_unique_address, limit=10)
        df = self.flipside_api.execute_query(sql)
        self.assertEqual(10, df.shape[0])
